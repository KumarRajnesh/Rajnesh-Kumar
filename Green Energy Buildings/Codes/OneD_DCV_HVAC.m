...
% Parameters
n_vent(1:14) = 0.35/3600; % number of air shifts
k_vent = 1.25;
k_vent_winter = 1;
...
for n = 1:length(Time)
    ...
    Q_vent(n,1) = cp_air_layer*rho_air_layer*(area_room_floor(1)*average_height_room)*n_vent(n,1)*(T_ext_interpolated(n,1)-T_int(n,1));
    ...
    Q_rad_glazed(n,1) = f_sh*(tau*area_wall_outer(1)*(1-f_facade_interpolated(n,1))*(1-f_curtains(n,1)))*G_sun_north_interpolated(n,1);
    ...
    f_curtains(n+1,1) = 0;
    %% Green building features: Room 1

    if Time_24(n)>=6 & Time_24(n)<=19
        if ismember(selected_month,[1,2,11,12])
            % f_curtains(n+1,14)=0;
            n_vent(n+1,1) = 0.35*k_vent_winter/3600;
            else
        if T_int(n,1) > T_comfortable + T_DCV
            % f_curtains(n+1,1)=0.5;
            n_vent(n+1,1) = 0.35*k_vent/3600;
            T_int(n+1,1) = T_int(n,1) + delta_time*3600/C_room(1)*(Q_vent(n,1)+Q_cond_conv(n,1)+Q_int_people(n,1)+Q_int_equip(n,1)+Q_rad_opaque_brick(n,1)+Q_rad_opaque_roof(n,1)+Q_rad_glazed(n,1));
        else
            % f_curtains(n+1,1)=0;
            n_vent(n+1,1) = 0.35/3600;
        end
        end

        if T_int(n,1) > T_comfortable + T_HVAC
            % n_vent(n+1,1) = 0.35/3600;
            T_int(n+1,1) = T_int(n,1) - T_change;
        
        elseif T_int(n,1) < T_comfortable - T_HVAC
            % n_vent(n+1,1) = 0.35/3600;
            T_int(n+1,1) = T_int(n,1) + T_change;
        ...
        Q_HVAC(n,1) = (C_room(1)*(T_int(n+1,1)-T_int(n,1))/(delta_time*3600)-(Q_vent(n,1)+Q_cond_conv(n,1)+Q_int_people(n,1)+Q_int_equip(n,1)+Q_rad_opaque_brick(n,1)+Q_rad_opaque_roof(n,1)+Q_rad_glazed(n,1)));
    ...
    Q_vent(n,2) = cp_air_layer*rho_air_layer*(area_room_floor(2)*average_height_room)*n_vent(n,2)*(T_ext_interpolated(n,1)-T_int(n,2));

    Q_rad_opaque_roof(n,2) = f_sh*(alpha_roof*R_se*U_roof_opaque*area_room_floor(2))*G_sun_roof_interpolated(n,1);

    Q_rad_glazed(n,2) = f_sh*(tau*area_wall_outer(2)*(1-f_facade_interpolated(n,1))*(1-f_curtains(n,2)))*G_sun_north_interpolated(n,1);
    ...
    f_curtains(n+1,2) = 0;
    %% Green building features: Room 2

    if Time_24(n)>=6 & Time_24(n)<=19
        if ismember(selected_month,[1,2,11,12])
            % f_curtains(n+1,14)=0;
            n_vent(n+1,2) = 0.35*k_vent_winter/3600;
            else
        if T_int(n,2) > T_comfortable + T_DCV
            % f_curtains(n+1,2)=0.5;
            n_vent(n+1,2) = 0.35*k_vent/3600;
            T_int(n+1,2) = T_int(n,2) + delta_time*3600/C_room(2)*(Q_vent(n,2)+Q_cond_conv(n,2)+Q_int_people(n,2)+Q_int_equip(n,2)+Q_rad_opaque_roof(n,2)+Q_rad_glazed(n,2));
        else
            % f_curtains(n+1,2)=0;
            n_vent(n+1,2) = 0.35/3600;
        end
        end

        if T_int(n,2) > T_comfortable + T_HVAC
            % n_vent(n+1,2) = 0.35/3600;
            T_int(n+1,2) = T_int(n,2) - T_change;
        
        elseif T_int(n,2) < T_comfortable - T_HVAC
            % n_vent(n+1,2) = 0.35/3600;
            T_int(n+1,2) = T_int(n,2) + T_change;
        ...
        Q_HVAC(n,2) = (C_room(2)*(T_int(n+1,2)-T_int(n,2))/(delta_time*3600)-(Q_vent(n,2)+Q_cond_conv(n,2)+Q_int_people(n,2)+Q_int_equip(n,2)+Q_rad_opaque_roof(n,2)+Q_rad_glazed(n,2)));
    ...
    Q_vent(n,3) = cp_air_layer*rho_air_layer*(area_room_floor(3)*average_height_room)*n_vent(n,3)*(T_ext_interpolated(n,1)-T_int(n,3));

    Q_rad_opaque_roof(n,3) = f_sh*(alpha_roof*R_se*U_roof_opaque*area_room_floor(3))*G_sun_roof_interpolated(n,1);

    Q_rad_glazed(n,3) = f_sh*(tau*area_wall_outer(3)*(1-f_facade_interpolated(n,1))*(1-f_curtains(n,3)))*G_sun_north_interpolated(n,1);
    ...
    f_curtains(n+1,3) = 0;
    %% Green building features: Room 3

    if Time_24(n)>=6 & Time_24(n)<=19
        if ismember(selected_month,[1,2,11,12])
            % f_curtains(n+1,14)=0;
            n_vent(n+1,3) = 0.35*k_vent_winter/3600;
            else
        if T_int(n,3) > T_comfortable + T_DCV
            % f_curtains(n+1,3)=0.5;
            n_vent(n+1,3) = 0.35*k_vent/3600;
            T_int(n+1,3) = T_int(n,3) + delta_time*3600/C_room(3)*(Q_vent(n,3)+Q_cond_conv(n,3)+Q_int_people(n,3)+Q_int_equip(n,3)+Q_rad_opaque_roof(n,3)+Q_rad_glazed(n,3));
        else
            % f_curtains(n+1,3)=0;
            n_vent(n+1,3) = 0.35/3600;
        end
        end

        if T_int(n,3) > T_comfortable + T_HVAC
            % n_vent(n+1,3) = 0.35/3600;
            T_int(n+1,3) = T_int(n,3) - T_change;
        
        elseif T_int(n,3) < T_comfortable - T_HVAC
            % n_vent(n+1,3) = 0.35/3600;
            T_int(n+1,3) = T_int(n,3) + T_change;
        ...
        Q_HVAC(n,3) = (C_room(3)*(T_int(n+1,3)-T_int(n,3))/(delta_time*3600)-(Q_vent(n,3)+Q_cond_conv(n,3)+Q_int_people(n,3)+Q_int_equip(n,3)+Q_rad_opaque_roof(n,3)+Q_rad_glazed(n,3)));
    ...
    Q_vent(n,4) = cp_air_layer*rho_air_layer*(area_room_floor(4)*average_height_room)*n_vent(n,4)*(T_ext_interpolated(n,1)-T_int(n,4));

    Q_rad_opaque_roof(n,4) = f_sh*(alpha_roof*R_se*U_roof_opaque*area_room_floor(4))*G_sun_roof_interpolated(n,1);

    Q_rad_glazed(n,4) = f_sh*(tau*area_wall_outer(4)*(1-f_facade_interpolated(n,1))*(1-f_curtains(n,4)))*G_sun_north_interpolated(n,1);
    ...
    f_curtains(n+1,4) = 0;
    %% Green building features: Room 4

    if Time_24(n)>=6 & Time_24(n)<=19
        if ismember(selected_month,[1,2,11,12])
            % f_curtains(n+1,14)=0;
            n_vent(n+1,4) = 0.35*k_vent_winter/3600;
            else
        if T_int(n,4) > T_comfortable + T_DCV
            % f_curtains(n+1,4)=0.5;
            n_vent(n+1,4) = 0.35*k_vent/3600;
            T_int(n+1,4) = T_int(n,4) + delta_time*3600/C_room(4)*(Q_vent(n,4)+Q_cond_conv(n,4)+Q_int_people(n,4)+Q_int_equip(n,4)+Q_rad_opaque_roof(n,4)+Q_rad_glazed(n,4));
        else
            % f_curtains(n+1,4)=0;
            n_vent(n+1,4) = 0.35/3600;
        end
        end

        if T_int(n,4) > T_comfortable + T_HVAC
            % n_vent(n+1,4) = 0.35/3600;
            T_int(n+1,4) = T_int(n,4) - T_change;
        
        elseif T_int(n,4) < T_comfortable - T_HVAC
            % n_vent(n+1,4) = 0.35/3600;
            T_int(n+1,4) = T_int(n,4) + T_change;
        ...
        Q_HVAC(n,4) = (C_room(4)*(T_int(n+1,4)-T_int(n,4))/(delta_time*3600)-(Q_vent(n,4)+Q_cond_conv(n,4)+Q_int_people(n,4)+Q_int_equip(n,4)+Q_rad_opaque_roof(n,4)+Q_rad_glazed(n,4)));
    ...
    Q_vent(n,5) = cp_air_layer*rho_air_layer*(area_room_floor(5)*average_height_room)*n_vent(n,5)*(T_ext_interpolated(n,1)-T_int(n,5));

    Q_rad_opaque_roof(n,5) = f_sh*(alpha_roof*R_se*U_roof_opaque*area_room_floor(5))*G_sun_roof_interpolated(n,1);

    Q_rad_glazed(n,5) = f_sh*(tau*area_wall_outer(5)*(1-f_facade_interpolated(n,1))*(1-f_curtains(n,5)))*G_sun_north_interpolated(n,1);
    ...
    f_curtains(n+1,5) = 0;
    %% Green building features: Room 5

    if Time_24(n)>=6 & Time_24(n)<=19
        if ismember(selected_month,[1,2,11,12])
            % f_curtains(n+1,14)=0;
            n_vent(n+1,5) = 0.35*k_vent_winter/3600;
            else
        if T_int(n,5) > T_comfortable + T_DCV
            % f_curtains(n+1,5)=0.5;
            n_vent(n+1,5) = 0.35*k_vent/3600;
            T_int(n+1,5) = T_int(n,5) + delta_time*3600/C_room(5)*(Q_vent(n,5)+Q_cond_conv(n,5)+Q_int_people(n,5)+Q_int_equip(n,5)+Q_rad_opaque_roof(n,5)+Q_rad_glazed(n,5));
        else
            % f_curtains(n+1,5)=0;
            n_vent(n+1,5) = 0.35/3600;
        end
        end

        if T_int(n,5) > T_comfortable + T_HVAC
            % n_vent(n+1,5) = 0.35/3600;
            T_int(n+1,5) = T_int(n,5) - T_change;
        
        elseif T_int(n,5) < T_comfortable - T_HVAC
            % n_vent(n+1,5) = 0.35/3600;
            T_int(n+1,5) = T_int(n,5) + T_change;
        ...
        Q_HVAC(n,5) = (C_room(5)*(T_int(n+1,5)-T_int(n,5))/(delta_time*3600)-(Q_vent(n,5)+Q_cond_conv(n,5)+Q_int_people(n,5)+Q_int_equip(n,5)+Q_rad_opaque_roof(n,5)+Q_rad_glazed(n,5)));
    ...
    Q_vent(n,6) = cp_air_layer*rho_air_layer*(area_room_floor(6)*average_height_room)*n_vent(n,6)*(T_ext_interpolated(n,1)-T_int(n,6));

    Q_rad_opaque_roof(n,6) = f_sh*(alpha_roof*R_se*U_roof_opaque*area_room_floor(6))*G_sun_roof_interpolated(n,1);

    Q_rad_glazed(n,6) = f_sh*(tau*area_wall_outer(6)*(1-f_facade_interpolated(n,1))*(1-f_curtains(n,6)))*G_sun_north_interpolated(n,1);
    ...
    f_curtains(n+1,6) = 0;
    %% Green building features: Room 6

    if Time_24(n)>=6 & Time_24(n)<=19
        if ismember(selected_month,[1,2,11,12])
            % f_curtains(n+1,14)=0;
            n_vent(n+1,6) = 0.35*k_vent_winter/3600;
            else
        if T_int(n,6) > T_comfortable + T_DCV
            % f_curtains(n+1,6)=0.5;
            n_vent(n+1,6) = 0.35*k_vent/3600;
            T_int(n+1,6) = T_int(n,6) + delta_time*3600/C_room(6)*(Q_vent(n,6)+Q_cond_conv(n,6)+Q_int_people(n,6)+Q_int_equip(n,6)+Q_rad_opaque_roof(n,6)+Q_rad_glazed(n,6));
        else
            % f_curtains(n+1,6)=0;
            n_vent(n+1,6) = 0.35/3600;
        end
        end

        if T_int(n,6) > T_comfortable + T_HVAC
            % n_vent(n+1,6) = 0.35/3600;
            T_int(n+1,6) = T_int(n,6) - T_change;
        
        elseif T_int(n,6) < T_comfortable - T_HVAC
            % n_vent(n+1,6) = 0.35/3600;
            T_int(n+1,6) = T_int(n,6) + T_change;
        ...
        Q_HVAC(n,6) = (C_room(6)*(T_int(n+1,6)-T_int(n,6))/(delta_time*3600)-(Q_vent(n,6)+Q_cond_conv(n,6)+Q_int_people(n,6)+Q_int_equip(n,6)+Q_rad_opaque_roof(n,6)+Q_rad_glazed(n,6)));
    ...
    Q_vent(n,7) = cp_air_layer*rho_air_layer*(area_room_floor(7)*average_height_room)*n_vent(n,7)*(T_ext_interpolated(n,1)-T_int(n,7));
    ...
    Q_rad_glazed(n,7) = f_sh*(tau*area_wall_outer(7)*(1-f_facade_interpolated(n,1))*(1-f_curtains(n,7)))*G_sun_north_interpolated(n,1);
    ...
    f_curtains(n+1,7) = 0;
    %% Green building features: Room 7

    if Time_24(n)>=6 & Time_24(n)<=19
        if ismember(selected_month,[1,2,11,12])
            % f_curtains(n+1,14)=0;
            n_vent(n+1,7) = 0.35*k_vent_winter/3600;
        else
        if T_int(n,7) > T_comfortable + T_DCV
            % f_curtains(n+1,7)=0.5;
            n_vent(n+1,7) = 0.35*k_vent/3600;
            T_int(n+1,7) = T_int(n,7) + delta_time*3600/C_room(7)*(Q_vent(n,7)+Q_cond_conv(n,7)+Q_int_people(n,7)+Q_int_equip(n,7)+Q_rad_opaque_brick(n,7)+Q_rad_opaque_roof(n,7)+Q_rad_glazed(n,7));
        else
            % f_curtains(n+1,7)=0;
            n_vent(n+1,7) = 0.35/3600;
        end
        end

        if T_int(n,7) > T_comfortable + T_HVAC
            % n_vent(n+1,7) = 0.35/3600;
            T_int(n+1,7) = T_int(n,7) - T_change;
        
        elseif T_int(n,7) < T_comfortable - T_HVAC
            % n_vent(n+1,7) = 0.35/3600;
            T_int(n+1,7) = T_int(n,7) + T_change;
        ...
        Q_HVAC(n,7) = (C_room(7)*(T_int(n+1,7)-T_int(n,7))/(delta_time*3600)-(Q_vent(n,7)+Q_cond_conv(n,7)+Q_int_people(n,7)+Q_int_equip(n,7)+Q_rad_opaque_brick(n,7)+Q_rad_opaque_roof(n,7)+Q_rad_glazed(n,7)));
    ...
    Q_vent(n,8) = cp_air_layer*rho_air_layer*(area_room_floor(8)*height_wall)*n_vent(n,8)*(T_ext_interpolated(n,1)-T_int(n,8));
    ...
    Q_vent(n,9) = cp_air_layer*rho_air_layer*(area_room_floor(9)*average_height_room)*n_vent(n,9)*(T_ext_interpolated(n,1)-T_int(n,9));
    ...
    Q_rad_glazed(n,9) = f_sh*(tau*area_wall_outer(9)*(1-f_facade_interpolated(n,1))*(1-f_curtains(n,9)))*G_sun_south_interpolated(n,1);
    ...
    f_curtains(n+1,9) = 0;
%% Green building features: Room 9

    if Time_24(n)>=6 & Time_24(n)<=19
        if ismember(selected_month,[1,2,11,12])
            % f_curtains(n+1,9)=0;
            n_vent(n+1,9) = 0.35*k_vent_winter/3600;
        else
        if T_int(n,9) > T_comfortable + T_DCV
            %f_curtains(n+1,9)=0.5;
            n_vent(n+1,9) = 0.35*k_vent/3600;
            T_int(n+1,9) = T_int(n,9) + delta_time*3600/C_room(9)*(Q_vent(n,9)+Q_cond_conv(n,9)+Q_int_people(n,9)+Q_int_equip(n,9)+Q_rad_opaque_brick(n,9)+Q_rad_opaque_roof(n,9)+Q_rad_glazed(n,9));
        else
            %f_curtains(n+1,9)=0;
            n_vent(n+1,9) = 0.35/3600;
        end
        end

        if T_int(n,9) > T_comfortable + T_HVAC
            % n_vent(n+1,9) = 0.35/3600;
            T_int(n+1,9) = T_int(n,9) - T_change;
        
        elseif T_int(n,9) < T_comfortable - T_HVAC
            % n_vent(n+1,9) = 0.35/3600;
            T_int(n+1,9) = T_int(n,9) + T_change;
        ...
        Q_HVAC(n,9) = (C_room(9)*(T_int(n+1,9)-T_int(n,9))/(delta_time*3600)-(Q_vent(n,9)+Q_cond_conv(n,9)+Q_int_people(n,9)+Q_int_equip(n,9)+Q_rad_opaque_brick(n,9)+Q_rad_opaque_roof(n,9)+Q_rad_glazed(n,9)));
    ...
    Q_vent(n,10) = cp_air_layer*rho_air_layer*(area_room_floor(10)*average_height_room)*n_vent(n,10)*(T_ext_interpolated(n,1)-T_int(n,10));

    Q_rad_opaque_roof(n,10) = f_sh*(alpha_roof*R_se*U_roof_opaque*area_room_floor(10))*G_sun_roof_interpolated(n,1);

    Q_rad_glazed(n,10) = f_sh*(tau*area_wall_outer(10)*(1-f_facade_interpolated(n,1))*(1-f_curtains(n,10)))*G_sun_south_interpolated(n,1);
    ...
    f_curtains(n+1,10) = 0;
%% Green building features: Room 10

    if Time_24(n)>=6 & Time_24(n)<=19
        if ismember(selected_month,[1,2,11,12])
            % f_curtains(n+1,10)=0;
            n_vent(n+1,10) = 0.35*k_vent_winter/3600;
        else
        if T_int(n,10) > T_comfortable + T_DCV
            %f_curtains(n+1,10)=0.5;
            n_vent(n+1,10) = 0.35*k_vent/3600;
            T_int(n+1,10) = T_int(n,10) + delta_time*3600/C_room(10)*(Q_vent(n,10)+Q_cond_conv(n,10)+Q_int_people(n,10)+Q_int_equip(n,10)+Q_rad_opaque_roof(n,10)+Q_rad_glazed(n,10));
        else
            %f_curtains(n+1,10)=0;
            n_vent(n+1,10) = 0.35/3600;
        end
        end

        if T_int(n,10) > T_comfortable + T_HVAC
            % n_vent(n+1,10) = 0.35/3600;
            T_int(n+1,10) = T_int(n,10) - T_change;
        
        elseif T_int(n,10) < T_comfortable - T_HVAC
            % n_vent(n+1,10) = 0.35/3600;
            T_int(n+1,10) = T_int(n,10) + T_change;
        ...
        Q_HVAC(n,10) = C_room(10)*(T_int(n+1,10)-T_int(n,10))/(delta_time*3600)-(Q_vent(n,10)+Q_cond_conv(n,10)+Q_int_people(n,10)+Q_int_equip(n,10)+Q_rad_opaque_roof(n,10)+Q_rad_glazed(n,10));
    ...
    Q_vent(n,11) = cp_air_layer*rho_air_layer*(area_room_floor(11)*average_height_room)*n_vent(n,11)*(T_ext_interpolated(n,1)-T_int(n,11));

    Q_rad_opaque_roof(n,11) = f_sh*(alpha_roof*R_se*U_roof_opaque*area_room_floor(11))*G_sun_roof_interpolated(n,1);

    Q_rad_glazed(n,11) = f_sh*(tau*area_wall_outer(11)*(1-f_facade_interpolated(n,1))*(1-f_curtains(n,11)))*G_sun_south_interpolated(n,1);
    ...
    f_curtains(n+1,11) = 0;
%% Green building features: Room 11

    if Time_24(n)>=6 & Time_24(n)<=19
        if ismember(selected_month,[1,2,11,12])
            % f_curtains(n+1,11)=0;
            n_vent(n+1,11) = 0.35*k_vent_winter/3600;
        else
        if T_int(n,11) > T_comfortable + T_DCV
            %f_curtains(n+1,11)=0.5;
            n_vent(n+1,11) = 0.35*k_vent/3600;
            T_int(n+1,11) = T_int(n,11) + delta_time*3600/C_room(11)*(Q_vent(n,11)+Q_cond_conv(n,11)+Q_int_people(n,11)+Q_int_equip(n,11)+Q_rad_opaque_roof(n,11)+Q_rad_glazed(n,11));
        else
            %f_curtains(n+1,11)=0;
            n_vent(n+1,11) = 0.35/3600;
        end
        end

        if T_int(n,11) > T_comfortable + T_HVAC
            % n_vent(n+1,11) = 0.35/3600;
            T_int(n+1,11) = T_int(n,11) - T_change;
        
        elseif T_int(n,11) < T_comfortable - T_HVAC
            % n_vent(n+1,11) = 0.35/3600;
            T_int(n+1,11) = T_int(n,11) + T_change;
        ...
        Q_HVAC(n,11) = (C_room(11)*(T_int(n+1,11)-T_int(n,11))/(delta_time*3600))-(Q_vent(n,11)+Q_cond_conv(n,11)+Q_int_people(n,11)+Q_int_equip(n,11)+Q_rad_opaque_roof(n,11)+Q_rad_glazed(n,11));
    ...
    Q_vent(n,12) = cp_air_layer*rho_air_layer*(area_room_floor(12)*average_height_room)*n_vent(n,12)*(T_ext_interpolated(n,1)-T_int(n,12));
    ...
    Q_vent(n,13) = cp_air_layer*rho_air_layer*(area_room_floor(13)*average_height_room)*n_vent(n,13)*(T_ext_interpolated(n,1)-T_int(n,13));

    Q_rad_opaque_roof(n,13) = f_sh*(alpha_roof*R_se*U_roof_opaque*area_room_floor(13))*G_sun_roof_interpolated(n,1);

    Q_rad_glazed(n,13) = f_sh*(tau*area_wall_outer(13)*(1-f_facade_interpolated(n,1))*(1-f_curtains(n,13)))*G_sun_south_interpolated(n,1);
    ...
    f_curtains(n+1,13) = 0;
%% Green building features: Room 13

    if Time_24(n)>=6 & Time_24(n)<=19
        if ismember(selected_month,[1,2,11,12])
            % f_curtains(n+1,13)=0;
            n_vent(n+1,13) = 0.35*k_vent_winter/3600;
        else
        if T_int(n,13) > T_comfortable + T_DCV
            %f_curtains(n+1,13)=0.5;
            n_vent(n+1,13) = 0.35*k_vent/3600;
            T_int(n+1,13) = T_int(n,13) + delta_time*3600/C_room(13)*(Q_vent(n,13)+Q_cond_conv(n,13)+Q_int_people(n,13)+Q_int_equip(n,13)+Q_rad_opaque_roof(n,13)+Q_rad_glazed(n,13));
        else
            %f_curtains(n+1,13)=0;
            n_vent(n+1,13) = 0.35/3600;
        end
        end

        if T_int(n,13) > T_comfortable + T_HVAC
            % n_vent(n+1,13) = 0.35/3600;
            T_int(n+1,13) = T_int(n,13) - T_change;
        
        elseif T_int(n,13) < T_comfortable - T_HVAC
            % n_vent(n+1,13) = 0.35/3600;
            T_int(n+1,13) = T_int(n,13) + T_change;
        ...
        Q_HVAC(n,13) = (C_room(13)*(T_int(n+1,13)-T_int(n,13))/(delta_time*3600))-(Q_vent(n,13)+Q_cond_conv(n,13)+Q_int_people(n,13)+Q_int_equip(n,13)+Q_rad_opaque_roof(n,13)+Q_rad_glazed(n,13));
    ...
    Q_vent(n,14) = cp_air_layer*rho_air_layer*(area_room_floor(14)*average_height_room)*n_vent(n,14)*(T_ext_interpolated(n,1)-T_int(n,14));
    ...
    Q_rad_glazed(n,14) = f_sh*(tau*area_wall_outer(14)*(1-f_facade_interpolated(n,1))*(1-f_curtains(n,14)))*G_sun_south_interpolated(n,1);
    ...
    f_curtains(n+1,14) = 0;
%% Green building features: Room 14

    if Time_24(n)>=6 & Time_24(n)<=19
        if ismember(selected_month,[1,2,11,12])
            % f_curtains(n+1,14)=0;
            n_vent(n+1,14) = 0.35*k_vent_winter/3600;
        else
        if T_int(n,14) > T_comfortable + T_DCV
            %f_curtains(n+1,14)=0.5;
            n_vent(n+1,14) = 0.35*k_vent/3600;
            T_int(n+1,14) = T_int(n,14) + delta_time*3600/C_room(14)*(Q_vent(n,14)+Q_cond_conv(n,14)+Q_int_people(n,14)+Q_int_equip(n,14)+Q_rad_opaque_brick(n,14)+Q_rad_opaque_roof(n,14)+Q_rad_glazed(n,14));
        else
            %f_curtains(n+1,14)=0;
            n_vent(n+1,14) = 0.35/3600;
        end
        end

        if T_int(n,14) > T_comfortable + T_HVAC
            % n_vent(n+1,14) = 0.35/3600;
            T_int(n+1,14) = T_int(n,14) - T_change;
        
        elseif T_int(n,14) < T_comfortable - T_HVAC
            % n_vent(n+1,14) = 0.35/3600;
            T_int(n+1,14) = T_int(n,14) + T_change;
        ...
        Q_HVAC(n,14) = (C_room(14)*(T_int(n+1,14)-T_int(n,14))/(delta_time*3600))-(Q_vent(n,14)+Q_cond_conv(n,14)+Q_int_people(n,14)+Q_int_equip(n,14)+Q_rad_opaque_brick(n,14)+Q_rad_opaque_roof(n,14)+Q_rad_glazed(n,14));
    ...
    Q_HVAC_sum(n,1) = sum(abs(Q_HVAC(n,:)));
    Q_HVAC_forsara(n,1) = sum(Q_HVAC(n,:));

    Energy_HVAC = sum(Q_HVAC_sum)*delta_time/1000

end
Time=[Time, 24*days+delta_time];
T_ext_interpolated(n+1)=0.5*(T_ext_interpolated(1)+T_ext_interpolated(n));

capacity_heatpump = max(abs(Q_HVAC_sum)/1000)
...
% 
% % % figure(7)
% % % plot(Time(1:end-1), Q_HVAC(:,1))
% % % 
% % % figure(8)
% % % plot(Time(1:end-1), Q_HVAC(:,2))
% % % 
% % % figure(9)
% % % plot(Time(1:end-1), Q_HVAC(:,3))
% % % 
% % % figure(10)
% % % plot(Time(1:end-1), Q_HVAC(:,4))
% % % 
% % % figure(11)
% % % plot(Time(1:end-1), Q_HVAC(:,5))
% % % 
% % % figure(12)
% % % plot(Time(1:end-1), Q_HVAC(:,6))
% % % 
% % % figure(13)
% % % plot(Time(1:end-1), Q_HVAC(:,7))
% % % 
% % % figure(14)
% % % plot(Time(1:end-1), Q_HVAC(:,8))
% % % 
% % % figure(15)
% % % plot(Time(1:end-1), Q_HVAC(:,9))
% % % 
% % % figure(16)
% % % plot(Time(1:end-1), Q_HVAC(:,10))
% % % 
% % % figure(17)
% % % plot(Time(1:end-1), Q_HVAC(:,11))
% % % 
% % % figure(19)
% % % plot(Time(1:end-1), Q_HVAC(:,13))
% % % 
% % % figure(20)
% % % plot(Time(1:end-1), Q_HVAC(:,14))
% % % 
% % % figure(21)
% % % plot(Time(1:end-1), Q_HVAC_sum)
% 
% figure(7)
% plot(Time(1:end-1), Q_vent(:,1))
% figure(8)
% plot(Time(1:end-1), Q_vent(:,2))
% figure(9)
% plot(Time(1:end-1), Q_vent(:,3))
% figure(10)
% plot(Time(1:end-1), Q_vent(:,4))
% figure(11)
% plot(Time(1:end-1), Q_vent(:,5))
% figure(12)
% plot(Time(1:end-1), Q_vent(:,6))
% figure(13)
% plot(Time(1:end-1), Q_vent(:,7))
% figure(14)
% plot(Time(1:end-1), Q_vent(:,8))
% figure(15)
% plot(Time(1:end-1), Q_vent(:,9))
% figure(16)
% plot(Time(1:end-1), Q_vent(:,10))
% figure(17)
% plot(Time(1:end-1), Q_vent(:,11))
% figure(18)
% plot(Time(1:end-1), Q_vent(:,12))
% figure(19)
% plot(Time(1:end-1), Q_vent(:,13))
% figure(20)
% plot(Time(1:end-1), Q_vent(:,14))
